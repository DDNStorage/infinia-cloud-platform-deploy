...
if [ ! -f $LOG_COMPLETE ] ; then 
   ...
   if systemctl list-units --type=service | grep -q lldpd.service; then 
     systemctl enable lldpd  --now  | tee -a "$LOG_FILE"
  fi
  ...
  cd /tmp 
    ...
    redcli realm config generate | tee -a $LOG_FILE || echo "Error: redcli config generate failed" tee -a $LOG_FILE
    redcli realm config update -f /tmp/realm_config.yaml | tee -a "$LOG_FILE" || echo "Error: redcli config update failed" | tee -a "$LOG_FILE"
    redcli license install -a '1DE94FE1-BE7D-4A4B-8DA2-7761ED7B66EA' -y | tee -a $LOG_FILE
    redcli cluster create ${var.infinia_deployment_name} -S=false -z  |  tee -a "$LOG_FILE" || echo "Error: failed to create cluster" | tee -a "$LOG_FILE"
